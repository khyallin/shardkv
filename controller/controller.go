...
func (sck *Controller) InitConfig(cfg *config.Config) {
	for {
		...
		if err == model.ErrMaybe {
			value, _, err := sck.Get("config")
			if err == model.OK && value == cfg.String() {
				break
			}
			continue
		}
...
func (sck *Controller) getCurConfig() (*config.Config, model.Tversion) {
...
func (sck *Controller) getNextConfig() (*config.Config, model.Tversion) {
	value, version, err := sck.Get("next")
	if err == model.ErrNoKey || value == "" {
		return nil, version
	}
	if err != model.OK {
...
func (sck *Controller) putCurConfig(cfg *config.Config, version model.Tversion) model.Err {
...
func (sck *Controller) putNextConfig(cfg *config.Config, version model.Tversion) model.Err {
...
func (sck *Controller) deleteNextConfig(version model.Tversion) model.Err {
