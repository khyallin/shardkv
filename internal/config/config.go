...
func analyze(c *Config) (Tgid, int, Tgid, int) {
	counts := map[Tgid]int{}
...
func (cfg *Config) Join(servers map[Tgid][]string) bool {
	changed := false
...
func (cfg *Config) Leave(gids []Tgid) bool {
	changed := false
...
func (cfg *Config) JoinBalance(servers map[Tgid][]string) bool {
	if !cfg.Join(servers) {
		return false
	}
	cfg.Rebalance()
...
func (cfg *Config) LeaveBalance(gids []Tgid) bool {
	if !cfg.Leave(gids) {
		return false
	}
	cfg.Rebalance()
...
func (cfg *Config) GidServers(sh Tshid) (Tgid, []string, bool) {
	gid := cfg.Shards[sh]
	srvs, ok := cfg.Groups[gid]
	return gid, srvs, ok
}

func (cfg *Config) IsMember(gid Tgid) bool {
...
func (cfg *Config) CheckConfig(t *testing.T, groups []Tgid) {
